...
- **`build_name_index` reports `truncation_reason: "timeout"`.** The tree is larger than a single 5-minute walk can cover; subsequent passes converge. The background refresher (every 6 hours) eventually stitches the whole index together.
