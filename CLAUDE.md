...
- **`workflowy_status.degraded_kind` classifies the cause of `degraded`.** Possible values: `null` (status is ok), `"auth"` (recent 401/403), `"cancelled"` (probe cancelled mid-flight), `"local_queue_wedged"` (probe timed out AND in-flight walks > 0 — local rate limiter is the bottleneck), `"upstream_unreachable"` (probe timed out with no in-flight work — upstream is down), or `"upstream_error"` (any other non-success probe outcome). Surfaced on the 2026-05-19 user-report recommendation: pre-fix, `status: "degraded"` alone gave the caller no remediation signal — wait-and-retry vs restart-the-server were indistinguishable. Pinned by `workflowy_status_emits_degraded_kind_field` and `classify_degraded_kind_distinguishes_local_queue_from_upstream`.
