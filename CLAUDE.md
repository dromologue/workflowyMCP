...
- `resolve_link` always returns `Ok` with a structured payload — on hit `{id, name, description, parent_id, resolved_via}`; on miss `{resolved: null, short_hash, scope, nodes_walked, elapsed_ms, hint, truncated, truncation_limit, truncation_reason, truncation_recovery_hint}` so the caller branches on `resolved` instead of parsing error strings. Pre-2026-05-19 a partial-walk miss returned `Err(tool_invalid_params)` which recorded the call as a failure in the op log and flipped `degraded` via `last_unrecovered_failure` even though the upstream API was fine — only our walk budget had elapsed. The Ok+envelope shape is symmetric with every other walk-shaped tool. Pinned by `resolve_link_returns_ok_with_null_resolved_on_walk_miss`.
