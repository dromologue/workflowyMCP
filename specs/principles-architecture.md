...
- **Every walk level is re-keyed to document order before it is emitted.** Sorting each listing is necessary but not sufficient: `fetch_descendants` assembles a level from a `buffer_unordered` stream, so the sibling *blocks* land in completion order and the flat `nodes` array of `get_subtree` was non-deterministic between identical runs (2026-07-28). `WorkflowyClient::order_level_by_parent` sorts each level by its parent's index in the previous level — stable, so the funnel's within-parent priority order survives; unknown parents sort to the tail rather than being dropped. It must run at **both** emission sites (mid-level truncation and normal descend, the latter after branch recovery), so a truncated read is a prefix of the full one rather than a differently-ordered sample. Pinned by `subtree_walk_orders_every_level_before_it_becomes_output` (call-site count), plus two behavioural tests. **Rule for new walk shapes: any concurrency introduced over a sibling set owes a re-ordering step before its output is user-visible.** Sorting inputs and then racing them is not sorting.
- **Read behaviour is a cross-surface parity surface, not just a cross-binary one.** The MCP↔CLI parity invariant (Principle 5) covers two surfaces of *this* checkout. A third exists: `workflowy-connector`, a hand-maintained twin deployment. Every drift it has had was a read-behaviour fix that landed here and not there — first name-index search semantics, then the display-order sort, which for two weeks made `list_children` on the same parent return **opposite orders** depending on which surface asked, while writes agreed perfectly. Writes agreeing is what makes this class expensive to diagnose: the disagreement presents as a phantom inversion of a write that actually landed correctly. Guarded by the connector's `bin/check-twin-schema` (pre-push), which counts call sites in both checkouts and is proven red against the real seeded defect. **A read-behaviour fix in this repo is not done until the twin has it or the guard fails the twin's next push.**
