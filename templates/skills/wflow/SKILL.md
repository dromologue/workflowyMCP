...
Up to four MCP surfaces can reach the same Workflowy account: **this server** over stdio on the user's machine (`mcp__workflowy__*`), **the WorkFlowy CLI** (`mcp__workflowy-cli__*`, if `wf` is installed and wired in), **WorkFlowy's own desktop MCP** inside the desktop app (`mcp__workflowy-desktop__*`), and — if the user has deployed one — **a remote connector** for claude.ai web/mobile, which is this same server behind an HTTP shim. Decide by device first, then capability.

**If the WorkFlowy CLI is connected** (`mcp__workflowy-cli__*`, or a `wf` binary reachable from Bash), prefer it for plain lookups: search, find, read, context, todos, tags. It answers from a local SQLite full-text cache of the whole tree, so it costs no API call, cannot trip the rate limit, and returns each hit's ancestor path — none of which this server's name index can do. It is a **cache**, though: after a write, or when reading something changed on another device, run `wf cache:sync` first or pass `--live`. Keep using this server for the second-brain layer (`$SECONDBRAIN_DIR` drafts, session logs, memory), mirror discipline, `review`, and scheduled work — the CLI has none of that.

> **Decide in one line.** On the user's machine: **use this server** unless the task needs attachments, client navigation, live-watch, or a rate-limit-free whole-tree read — those go to the desktop MCP, then come straight back. **Do not use a remote connector from a machine that can reach this server**: they share one Workflowy account and therefore one API rate limit, so local traffic routed remotely doubles the load on a single budget and burns the quota the mobile surface depends on. If the desktop MCP is unavailable, fall back to this server automatically; never fall outward to the connector.
