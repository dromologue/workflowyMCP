...
**Native live mirrors are beta-only; the convention stays the production path.** `wf mirror:create` / `mirror:info` / `mirror:remove` drive Workflowy's real mirror primitive, but only against the beta API — the same call on production returns `beta_api_required`, and beta-created mirrors do not render coherently on production. Keep using `create_mirror`'s `mirror_of:` / `canonical_of:` convention for anything the user relies on, and treat `wf --beta mirror:*` as an experiment, never as the default. Note also that the drift audit answers questions a live mirror does not make redundant: whether the canonical is marked, whether it still exists, and whether this claim is a substantive contribution to the region it is mirrored into.
