...
- **`wf` CLI** (`mcp__workflowy-cli__*`, or a `wf` binary reachable from Bash) — search, find, read, context, todos, tags, ordinary writes, and nested outline writes via `edit_doc`. It answers from a local SQLite full-text cache of the whole tree, so plain reads cost no API call, cannot trip the rate limit, and return each hit's ancestor path, none of which this server's name index does. It is a **cache**, and refreshing it is expensive — see the timeout rule below before you reach for `cache:sync` or `--live`.
- **Desktop MCP** (`mcp__workflowy-desktop__*`) — prefer it during a live desktop session for anything heavy, because it drives the already-synced client and draws on **no REST quota at all**. `tree_seek` traverses from any anchor, upward or downward-deep, filtered by regular expression, text, creation/modification/completion window, URL, child count, or id, and can block until a match appears (`wait_any_match`). `tree_patch` applies a batch of nested inserts, moves, deletes, completions and renames in one call, optionally under **advisory leases** (`lease_acquire` / `lease_touch` / `lease_release`) — the only concurrency primitive available on any surface, and worth taking when two writers might touch the same node. It also owns **attachments** (`attachments_attach` / `attachments_pull`), **client navigation** (`tree_zoom`) and `screencap_capture`, none of which this server has any substitute for. Its ids are 12-char handles, not full UUIDs.

**Come back to this server for** the second-brain layer (`$SECONDBRAIN_DIR` drafts, session logs, memory canonicals), mirror discipline (`create_mirror` / `audit_mirrors`), `review`, the persistent name index with its privacy exclusions, headless and scheduled work (`wflow-do`), and any task where a typed failure envelope matters. No native surface has any of that.

**Native live mirrors are half-shipped: the write works on production, the read does not (measured 2026-09-12).** Earlier versions of this skill said mirrors were beta-only. That is wrong about the write: `POST /nodes/{id}/mirror` against production creates a real mirror and returns `{item_id, origin_id}`. What is beta-only is the **linkage that identifies a node as a mirror** — read the same mirror back on production and it has an empty name and no `data.mirror`, while on beta it carries the origin's name and the linkage in both directions. (`wf mirror:info` refuses production with `beta_api_required`, but that is `wf`'s own gate, not the API's.)

So keep using `create_mirror`'s `mirror_of:` / `canonical_of:` convention for anything the user relies on, and treat a native mirror as an experiment. The reason is now stronger than "beta": a second brain built on native mirrors would be **invisible to every tool that reads the production API**, this server and its drift audit included. Two consequences worth carrying:

- **A native mirror reads back as an empty-named node on production.** That makes it indistinguishable from the empty-named capture artefact that inbox triage offers to sweep. Never bulk-delete empty-named nodes without checking whether the user has been experimenting with `wf --beta mirror:create`.
- **The drift audit answers questions a live mirror does not make redundant:** whether the canonical is marked, whether it still exists, and whether this claim is a substantive contribution to the region it is mirrored into. Those survive the mechanism changing.

Watch for one narrow, testable condition: `data.mirror` appearing on a **production** read, and a mirror returning its origin's name rather than an empty string. When both hold, migrating is the right move.

**Calendar targets replace date-node arithmetic, and this server cannot reach them (measured 2026-09-12).** Production accepts a target key wherever a `parent_id` is expected — `inbox`, `today`, `tomorrow`, `next_week`, and literal `YYYY`, `YYYY-MM`, `YYYY-MM-DD` — and on a **create** it materialises whatever part of its own calendar is missing. A probe creating one node under `2027-03-09` produced the whole chain, `Journal → 2027 → March → <time …>Mar 9, 2027</time>`, under the account's own Journal.

This matters because of what it removes. A journal writer that resolves year, then month, then day, creating each missing level itself, ends up creating **its own** date node carrying `<time>` markup rather than Workflowy's. The client keys date-node adoption on metadata the REST API cannot set, so such a node is never adopted — it appears beside the native one as a Found Date, and converging the two is a manual drag by the user. Under a calendar target the node you are handed **is** the native one, so the twin cannot arise at all.

Three things to hold:

- A **read** resolves but never creates. `GET /nodes/today` returns the existing date node; asking for children of a date key 404s until something has materialised it.
- **`mcp__workflowy__*` rejects target keys.** This server validates a node id as a UUID or a hex short hash and refuses anything else at the handler boundary, so `parent_id: "today"` fails before a request is built. Reach for `wf` (which addresses them as `@inbox`, `@today`) or the raw API. The desktop MCP, which works in 12-character handles, cannot address them either.
- Where a user's journal already carries twin date nodes from an earlier tool, **leave them alone**: never merge twins and never delete a date node. Report more than one date node for a day as the expected Found-Date state rather than as a fault, and let the user converge them by hand.

**When `wf` hangs, it is syncing — stop waiting and use this server (measured 2026-09-07).** A `wf` read from cache answers in well under a second. A `wf` **sync** re-exports the entire tree: **22.6 s cold, and 56.1 s when it lands inside Workflowy's export throttle**, because that endpoint has a floor of roughly 65 s between calls and the second request simply waits it out. Either figure is long enough to blow an MCP host's tool timeout, and the `wf` MCP server **refreshes the cache by itself** when it judges the cache stale or a lookup out of date — so an innocuous `search` can silently become a minute-long whole-tree export with nothing in the response saying so.

Three rules follow, and the first is the one that recovers a stuck session:

1. **Treat a `wf` call still running after ~15 s as a sync in progress, not a slow search.** Do not wait for it and do not retry it — a retry queues behind the same throttle and takes longer. Re-issue the read against `mcp__workflowy__*` (`get_node`, `read_batch`, or `search_nodes` / `find_node` — **`use_index=true` when unscoped, `prefer_index=true` only when you also pass `parent_id`**, since both refuse a root walk otherwise). It is REST-backed and needs no cache. Note the fallback in one line; it is never a reason to fail a task.

   **The fallback is a stricter matcher, so a miss is not proof of absence.** `wf` does fuzzy full-text; the name index does a token-AND match, so every whitespace-delimited term must appear literally. A hyphenated query is one token: `claim-based second brain` returns nothing from the index while `wf` finds it. Drop compound terms and retry, or scope with `parent_id` and take the live walk. The index is also smaller than the tree by design (50,756 entries against 66,409 nodes on 2026-09-07) because the privacy exclusions keep whole subtrees off it, so material there is invisible to this path and reachable only by a live scoped read.
2. **Never call the `sync` tool through MCP.** If the cache genuinely must be refreshed, run `wf cache:sync` from Bash, where no tool timeout applies, and never twice inside a minute — a back-to-back pair is exactly the 56 s case above.
3. **After your own write, read back through this server rather than syncing `wf`.** `get_node` / `read_batch` are live by definition, so the verify-after-write step costs one call instead of a whole-tree export. Syncing `wf` to confirm a write you just made is the most common way to trigger this hang, and it is never necessary.

`--live` is the exception that is usually fine: it bypasses the cache for one query without re-exporting anything (measured 5.3 s). Reach for it when freshness matters for a single read; reach for a sync almost never.

**A native surface going away is never a reason to fail a task.** The desktop MCP exists only while the Workflowy desktop app is running, so its absence (missing from the tool list, or a call erroring/timing out) is expected, not exceptional; the same goes for a `wf` binary that is not installed. For anything this server can also do, fall back to it automatically rather than aborting or reporting failure, and note the fallback in one line so the caller knows a slower, rate-limited surface was used. Only attachments, `tree_zoom` and `screencap_capture` have no substitute here, and only those steps should ever come back as genuinely blocked.

**Child order — ascending `priority` is the ground truth.** Never infer outline order from the order a listing happens to arrive in. Workflowy's `/nodes?parent_id=` endpoint returns children in an internal/creation order, not display order; this server sorts every children listing ascending by `priority` (lowest renders at the top, priority-less nodes at the head) and re-keys each subtree walk level to its parent's position, so `list_children` and `get_subtree` agree with each other and with the outline. A deployment running an older build, or a different Workflowy client, may not do either — so when a list looks inverted, check `priority` before concluding a write went in backwards. If order genuinely matters, assert it with `reorder_nodes`, the only primitive that guarantees a sequence.
...
3. **`use_index=true` is the recovery for walk-budget timeouts on name queries — but the recovery has its own failure mode, and it is silent.** `find_node` and `search_nodes` answer from the persistent name index in O(1) with no walk budget. Index is name-only — descriptions need a live walk. Populate via `build_name_index(parent_id=<scope>)` once per fresh session or whenever the index is sparse. **On a large subtree `build_name_index` is itself what times out** (measured against one pillar: returned at exactly 20,000 ms with `truncated: true`, having indexed 246 nodes of far more), and an index query afterwards then answers confidently from partial coverage — a fast clean-looking hit that is really a miss. So: read the truncation envelope on the `build_name_index` response too, and if it truncated, treat every subsequent `use_index=true` answer on that scope as **unproven** rather than empty. The deep fix is an out-of-process `wflow-do reindex --timeout-secs 0 --patient --root <UUID>`, which needs a shell; where there is no shell, narrow the scope instead and say which branches went unindexed.
...
9. **Every error response is a typed envelope — branch on `proximate_cause`, not on the message string.** Failures carry `{operation, node_id, proximate_cause, retryable, retry_after_secs, hint, error}`. `proximate_cause` is one of `rate_limited` / `timeout` / `upstream_error` / `upstream_blocked` / `auth_failure` / `not_found` / `cancelled` / `lock_contention` / `cache_miss` / `invalid_params` / `unknown`. **`upstream_blocked` is the one people misdiagnose.** Workflowy can refuse every write while serving every read: a `403` carrying a `WFB(n)` code is an edge block that fires *before* authentication and is scoped to the HTTP method, so reads keep working and the credential is fine. It is `retryable: true` and the only correct response is to preserve the payload, report it, wait, and replay — never rotate a key, never redeploy, never retry in a loop. Classifying it as `auth_failure` sends the user to replace a key that is authenticating every read; the server puts the `upstream_blocked` branch ahead of the 401/403 branch precisely so that cannot happen. **`rate_limited` (429) is the one to act on:** `retryable:true` and `retry_after_secs:N` tell you to wait N seconds and re-issue the same call — it is a recoverable transient, NOT a failed write. Do not treat a bare top-line "tool failed" string (what some clients render) as undiagnosable; the structured `data` envelope carries the real cause. Pre-2026-06-17 a 429 was mislabelled `unknown` with the `retry_after` buried; it is now first-class. `retryable:false` (auth, invalid_params, unknown) means fix-and-retry or stop, never blind-retry.
10. **The write ok-counter is receipt, not commit — a live re-read is the only proof.** `per_tool_health.<tool>.ok` counts handler receipts; a rolled-back `transaction` and an `is_error:true` `insert_content` partial both record as `Err` (commit-accurate since 2026-06-01 / 2026-06-17), but a write whose response was lost to a transport timeout is genuinely ambiguous. **`create_node` takes an optional best-effort `idempotency_key`** (2026-06-17): pass a stable token to make a retry of that create safe — a repeated key replays the original node with an `idempotent_replay:` message instead of double-writing. It covers retry-after-success and retry-before-write, but NOT an ambiguous timeout after the write was sent (the server never recorded the success) — so for a single-node create lost to a timeout you STILL read back before retrying. It's server-process-scoped (the `wflow-do` CLI has no key — a one-shot process can't dedupe). `insert_content` / `batch_create_nodes` take no key — their committed-count cursor / per-op `succeeded` (contract 6) make resume safe by construction. Make verify-after-write the discipline for any write near a rate-limit window.
   ...
   **And the inverse is the more dangerous half: a failure report is not evidence the write failed.** Observed 2026-09-12 — a 68-node insert returned a session-expired error and the transport dropped, yet the whole block had landed. A retry would have duplicated it. So the rule is symmetric: **read the tree, not the response**, whichever way the response points. Where the read shows the write landed, say so and do not retry; where it shows a partial, resume from what is actually there rather than from the cursor the failed call reported.
...
### A second writer is possible; never delete what you find

The tree is the user's, and other things write to it — the user on a phone, a scheduled job, another agent. Where a workflow expects to create something and finds it already there, **do not delete or overwrite it.** Establish from its creation time whether it completed before this session began. If it did and it is sound, stand down on that write but still verify it and say what it missed. If it is still being written, or you cannot tell where it came from, **add yours alongside and record both ids** — standing down on a race risks losing the work to a writer that may never finish. Deleting the other writer's work is never the answer, because you cannot tell from the tree whether it was abandoned or merely slow.

Where the surface supports it, take the lock instead of reasoning about the race: the desktop MCP's advisory leases (`lease_acquire` / `lease_touch` / `lease_release`) are the only real concurrency primitive available, and worth using whenever two writers might touch the same node.
...
8. **Journal check-in** — append a dated entry under the Journal node. **Do not build the date node yourself.** Workflowy's calendar targets resolve or create the year/month/day chain natively: create with `parent_id: "today"` (or `"tomorrow"`, or a literal `YYYY-MM-DD`) through a surface that accepts target keys — `wf`, or the raw API — and insert under the id that comes back. The node you get is the client's own (a UUIDv5), which is the whole point. A date node you construct yourself is never adopted by the client and appears beside the native one as a Found Date the user has to drag. Where the user's journal already carries such twins from an earlier tool, **never merge them and never delete a date node**: report more than one date node for a day as the expected state and let the user converge them. And where you do have to create a date or month container yourself, read its parent afterwards — a freshly created container lands at an unpredictable priority among its siblings and routinely sorts above months that should precede it.
...
4. **"Empty" and "unproven" are different answers, and collapsing them is a lie.** A check that ran cleanly and found nothing is *empty*. A check whose walk truncated, whose index was built from partial coverage, or whose probe could not see the level the content lives on, is **unproven** — it found nothing and proved nothing. Report the second as unproven, name what stopped it, and say which scope was not covered. Unproven is an honest answer; empty is not. A session where nobody looked and a session where there was nothing to find must never produce the same report.
...
**Sequential content — numbered steps, ranked priorities, chronological entries, anything where position carries meaning — must read top-to-bottom in the intended order after the write.** This is not automatic. `insert_content`, `batch_create_nodes`, and any sequential `create_node` loop plant each new sibling at the *head* of the parent's children, so the last item created appears first and an N-line ordered list arrives **fully reversed** — step 5 above step 1, the narrative running backwards.

**An explicit `priority` is not a defence, and this is the correction that matters most (measured 2026-09-12).** Five creates under one fresh parent, carrying `priority` 100, 200, 300, 400, 500, were stored as **100, 50, 25, 12, 6** — Workflowy discarded the supplied values and applied its own halve-from-the-head rule, exactly as it does for a priority-less create. The list read back reversed. Earlier versions of this skill said siblings land at the head "unless an explicit `priority` is set"; that clause was false, and believing it is worse than not knowing, because it invites skipping the read-back. Send the priority if you like — it costs nothing and would take effect if the upstream ever begins honouring it — but **treat every multi-node write as unordered until a read-back says otherwise.**
...
2. **Verify after the write, not just before.** Read the parent back (`list_children` / `get_node`) and confirm the sequence reads in the intended direction. Reversal is the default failure, not an edge case.
3. **Check the grandparent too, whenever the write created a container.** Every check above is scoped *inside* the payload, and none of them sees the commonest real failure: the new container node itself landing in the wrong place among *its* siblings. A newly created month or date or project node arrives at an arbitrary priority and routinely sorts above nodes that should precede it. The priority value is noise — sightings of 0, 3, 6, 12, 25, 100 and 400 for the same defect, so there is no threshold to test. If the write created a container, read that container's parent as well.
4. **A loop that places nodes one at a time must run backwards.** Any operation that puts a node among siblings without an honoured priority puts it **first**, so iterating an ordered list forwards inverts it. Feed sibling groups reversed, or assert the order afterwards. This applies to a loop of moves just as much as to a loop of creates.
5. **Repair with `reorder_nodes(parent_id, node_ids[])`** — pass the node IDs in the intended top-to-bottom order. Its reverse-`priority=0` primitive (contract 8) lands them in that sequence regardless of how they currently sit. This is the canonical fix; never hand-roll a forward-priority loop. **Two traps:** it is built on `move_node`, so any id you name that is not currently under `parent_id` gets **reparented**, and its success report looks identical either way — so verify afterwards that nothing moved between levels. And `last_inserted_id` is not the block's root: where a payload was fed reversed, the node written last is the block's *first* bullet, so passing it to `reorder_nodes` lifts the opening line out of its block. Read the parent's children back and take the id of the node whose **name** is the block's top line.
6. **Prefer building the structure correctly in one pass** where the tool allows it — a single `insert_content` payload preserves the parent→child *hierarchy*, but sibling order within a level still inverts, so the read-back is mandatory regardless of which tool wrote the list.
7. **An order check has a shelf life.** Verifying order at write time says nothing about the tree an hour later, once the user has dragged things about. Do not re-assert order on old evidence; re-read.

The rule in one line: **an ordered list is not "inserted" until a read-back confirms it reads forwards — and the read-back must include the container's own position among its siblings.**
...
**Read `truncated` before `count`. A zero with `truncated: true` proves nothing.** This is the single easiest read error to make, because both answers look like "nothing there". A `count: 0` with `truncated: false` is a real proof of absence over the scope walked; the same `count: 0` with `truncated: true` says only that the walk stopped early, and the thing you were looking for may be sitting in the branches it never reached. Never report the second as "nothing found" — report it as **unproven**, and say which scope went unwalked.

**A walk that spills to a file and a walk that truncates arrive identically — and only one of them is an answer.** When a large read is too big for the response, the host writes it to a file and tells you where, with a note about reading it in chunks. A *complete* read that merely overflowed and a read that **hit the 20 s walk budget and stopped mid-branch** present the same way. The discriminator is the banner at the top of the spilled output, not the data below it: read the first line of the file before you read the JSON. If it is whole, do not chunk it. If it truncated, complete it with scoped reads of the containers the walk never reached, and name those containers in what you report.

**Where a walk truncates is not random, and it costs you the deepest content first.** A walk that stops early loses the most deeply nested material, which in most trees is exactly where the specific things live — a task filed under a person under a group sits several levels down and disappears before anything shallow does. Assume a truncated walk has lost the leaves, not a random sample.

**Two read tools under-report without erroring.** `since` reads a node's **own** `last_modified` and does not roll up its descendants, so it cannot see a change made to a child — and a probe run one level down has the same blindness one level further down again. Never use it to prove a subtree is unchanged. And `export_subtree` silently under-returns on a large subtree when the walk budget expires: measured returns of 736 of 1,050 nodes and 360 of 5,682, in both cases with no error. Reconcile a walk's node count against something independent before treating it as complete.

**A tool's `truncation_recovery_hint` names a recovery that exists on *some* surface, not necessarily yours.** Before following a hint twice, check whether the thing it recommends is reachable at all from where you are running — a hint pointing at a shell binary is useless in a sandbox with no shell.
...
When reads or mutations have failed in the last 30 s, a successful `create_node` response is suffixed with a `⚠ DEGRADED: server in degraded state — <tool> failed N ms ago …` banner.

**Read the cause before believing the banner.** The degraded-state detector counts *your own* bad parameter as upstream degradation: pass an invalid argument to `get_subtree`, and the next successful `create_node` twenty seconds later carries a degraded banner although nothing upstream was ever wrong (observed 2026-08-22). So check `proximate_cause` on the failure the banner names. **`invalid_params` is yours** — fix the call and carry on; the banner is noise. `timeout`, `upstream_error`, `rate_limited` and `upstream_blocked` are the estate's, and then the rule below applies. A degraded banner is a prompt to read a cause, never a verdict on its own.

When the cause is genuinely upstream, **do not chain follow-up writes on the new UUID** until `workflowy_status` shows the previously-failing tool back to `healthy`. Chaining `create → move → move → …` while the move path is wedged is what scatters orphaned nodes across the tree.
...
**A dead id is loud; a stale id is silent — and the second is what a cache re-verification cannot catch.** Confirming a cached id by fetching it proves only that *something* is there. Where a region has been rebuilt rather than edited — sections superseded and moved to an archive instead of deleted — every cached id still resolves, returns complete, well-formed, untruncated content, and is wrong. Nothing in the response says so. So where a surface is republished rather than edited in place, resolve identity **downward from the parent** (`list_children` from the region root) rather than from the cached list, and treat a well-formed answer from a cached id as unproven until its parentage has been checked.

**A measurement of a live tree expires.** Any figure recorded here about a tree the user is actively growing — a descendant count, "this fits in one call", a structure map — is a dated observation, not a standing fact. Record the date beside it, and treat a later run finding a different number as the note being out of date rather than as a fault to investigate.
